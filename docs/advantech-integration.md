...
```text
React 前端 -- HTTP / WebSocket --> Edge Gateway -- REST / MQTT / Modbus TCP --> WISE-4012、ADAM-6050
                                                     |
                                                   EKI-2525（仅工业交换机）
```
...
网关 API：`GET /api/v1/health`、`GET /api/v1/sensors/latest`、`GET /api/v1/devices`、`GET /api/v1/actuators`、`POST /api/v1/control/commands`。所有 DTO 均含 `schemaVersion: 1`，外部 JSON 必须运行时校验。传感器包会携带来源、设备、通道、原始值、工程值、单位、采集/接收时间和质量；只有 `quality=good` 且 `valid=true` 可进入自动控制。
...
默认 `VITE_RUNTIME_MODE=simulation`，`external` 必须配置网关 URL；未配置时明确显示 `unconfigured`，不静默回退。离线构建始终为本地模拟，不会请求网关。
